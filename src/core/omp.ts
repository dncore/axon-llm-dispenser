...
type OmpModelsMerge = { ops: TextOp[]; added: number; removed: number };

/** 在 provider 块内按 id 合并 models 列表(用户键保留;下架条目删除;新条目按字母序插入)。 */
function planOmpModelsMerge(
  text: string,
...
/** 应用已有 provider 块的管理键合并:scalarKeys(可为 null)+ models 列表。 */
function applyOmpProviderMerge(
  ...
  const merge = planOmpModelsMerge(text, providerBody, providerIndent, models);
...
/**
 * 「仅更新模型列表」:只刷新既有 providers.<name> 块的 models 列表(条目按 id 合并),
